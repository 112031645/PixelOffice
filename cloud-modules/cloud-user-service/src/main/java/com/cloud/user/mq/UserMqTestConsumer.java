...
/**
 * RocketMQ 联调测试消费者 —— 监听 {@link UserMqProducer#TOPIC_USER_TEST}。
 * 配合 DemoController 的 {@code POST /demo/mq/send} 验证「生产-消费」全链路。
 */
@Slf4j
@Component
@RocketMQMessageListener(
        topic = UserMqProducer.TOPIC_USER_TEST,
        consumerGroup = "${spring.application.name}-test-consumer-group"
)
public class UserMqTestConsumer implements RocketMQListener<String> {
